...
if (!function_exists('normalize_loopback_url')) {
    /**
     * 统一 localhost/127.0.0.1 的回环地址，避免本地访问域名不一致导致图片裂图
     * @param mixed $content
     * @param string $currentDomain
     * @return mixed
     */
    function normalize_loopback_url($content, string $currentDomain = '')
    {
        if (!$currentDomain) {
            try {
        ...
        if (is_array($content)) {
            foreach ($content as $key => $value) {
                $content[$key] = normalize_loopback_url($value, $currentDomain);
            }
            return $content;
        }
        ...
        $current = parse_url($currentDomain);
        if (!$current) {
            return $content;
        }
        ...
        return preg_replace_callback('/https?:\/\/(?:127\.0\.0\.1|localhost)(?::\d+)?/i', function ($matches) use ($origin, $current) {
            $target = parse_url($matches[0]);
            if (!$target) {
                return $matches[0];
            }
            $targetPort = $target['port'] ?? '';
            $currentPort = $current['port'] ?? '';
            if ((string)$targetPort !== (string)$currentPort) {
                return $matches[0];
            }
            return $origin;
        }, $content);
    ...
    function set_file_url($image, $siteUrl = '')
    {
        ...
        if (is_array($image)) {
            foreach ($image as &$item) {
                ...
                $item = normalize_loopback_url($item);
            }
        } else {
            ...
            $image = normalize_loopback_url($image);
        }
        return $image;
    }
}
