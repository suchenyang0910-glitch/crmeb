...
class ThemeServices extends BaseServices
{
    ...
    public function getThemeInfo($id, $type = 'all')
    {
        ...
        $info = $info->toArray();
        if ($type == 'home') {
            return normalize_loopback_url(json_decode($info['home_data'], true) ?? []);
        } elseif ($type == 'category') {
            return ['status' => $info['category_data'] ?? 1];
        } elseif ($type == 'detail') {
            return normalize_loopback_url(json_decode($info['detail_data'], true) ?? []);
        } elseif ($type == 'user') {
            return normalize_loopback_url(json_decode($info['user_data'], true) ?? []);
        } elseif ($type == 'theme') {
            ...
            return normalize_loopback_url(json_decode($info['theme_data'], true) ?? []);
        } elseif ($type == 'base') {
